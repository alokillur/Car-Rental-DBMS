...
--
ALTER TABLE `clientcars`
  ADD CONSTRAINT `clientcars_ibfk_1` FOREIGN KEY (`client_username`) REFERENCES `clients` (`client_username`) ON DELETE CASCADE,
  ADD CONSTRAINT `clientcars_ibfk_2` FOREIGN KEY (`car_id`) REFERENCES `cars` (`car_id` ON DELETE CASCADE;

--
-- Constraints for table `driver`
--
ALTER TABLE `driver`
  ADD CONSTRAINT `driver_ibfk_1` FOREIGN KEY (`client_username`) REFERENCES `clients` (`client_username`) ON DELETE CASCADE;

--
-- Constraints for table `rentedcars`
--
ALTER TABLE `rentedcars`
  ADD CONSTRAINT `rentedcars_ibfk_1` FOREIGN KEY (`customer_username`) REFERENCES `customers` (`customer_username`) ON DELETE CASCADE,
  ADD CONSTRAINT `rentedcars_ibfk_2` FOREIGN KEY (`car_id`) REFERENCES `cars` (`car_id`) ON DELETE CASCADE,
  ADD CONSTRAINT `rentedcars_ibfk_3` FOREIGN KEY (`driver_id`) REFERENCES `driver` (`driver_id`) ON DELETE CASCADE; 
COMMIT;
...
-- DELETE FROM customers WHERE customers_id = '&customer_id';



-- Update the car availability
UPDATE 'cars' SET 'car_availability'='yes' WHERE car_id = '&car_id';


-- Update the diver status
UPDATE 'drivers' SET 'driver_availability'='no' WHERE driver_id = '&driver_id';


-- Trigger to update car availability when a car is rented
CREATE TRIGGER update_car_availability_on_rent
AFTER INSERT ON rentedcars
FOR EACH ROW
BEGIN
    UPDATE cars
...
-- Trigger to update car availability when a car is returned
CREATE TRIGGER update_car_availability_on_return
AFTER UPDATE ON rentedcars
FOR EACH ROW
BEGIN
    IF NEW.return_status = 'R' THEN
        UPDATE cars
...
-- Stored procedure to delete a customer and update car availability
DELIMITER $$
CREATE PROCEDURE delete_customer_and_update_car_availability(
    IN customer_username VARCHAR(50)
)
BEGIN
    DECLARE car_id_var INT;
    
    -- Get the car ID associated with the customer
    SELECT car_id INTO car_id_var FROM rentedcars WHERE customer_username = customer_username;
    
    -- Update the car availability to 'yes'
    UPDATE cars SET car_availability = 'yes' WHERE car_id = car_id_var;
    
    -- Delete the customer from the customers table
    DELETE FROM customers WHERE customer_username = customer_username;
END$$
DELIMITER ;
...
SELECT * FROM rentedcars WHERE customer_username = 'desired_customer_username';

SELECT * FROM rentedcars rc
JOIN clientcars cc ON rc.car_id = cc.car_id
WHERE cc.client_username = 'desired_client_username';
